...
//	    	debugger
	        ...
	        if(rotates[direction]) {
	            $frameImg.data('width', rotates[direction].width*window.devicePixelRatio);
	            $frameImg.data('height', rotates[direction].height*window.devicePixelRatio);
	...
	function generatePoster() {
	    ...
	    canvas.width = $frame.width()*window.devicePixelRatio;//CSS中定义了画布是580
	    canvas.height = $frame.height()*window.devicePixelRatio;
	   ...
	   poster.drawImage(ctx, $word, poster.intersect($frame, $word));
	    poster.drawImage(ctx, $erweima, poster.intersect($frame, $erweima));
...
//	        return 1;
	    ...
	    filterImage: function(image, width, height, deg) {
	        ...
	        canvas.width = width / pr*window.devicePixelRatio;//恢复为原先的大小
	        canvas.height = height / pr*window.devicePixelRatio;
